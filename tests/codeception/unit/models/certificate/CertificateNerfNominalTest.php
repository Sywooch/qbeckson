...
class CertificateNerfNominalTest extends Unit
{
    ...
    {
        expect(
            'прошла инициализация, в нерфере модель сертификата',
            $this->nerfer->certificate instanceof Certificates
        )->true();
        expect(
            'прошла инициализация, id тот что указан',
            $this->nerfer->certificate->id
        )->equals(self::CERTIFICATE_PF_ID);
        expect(
            'прошла инициализация, в нерфере модель сертификата учета',
            $this->nerferSpecail->certificate instanceof Certificates
        )->true();
        expect(
            'прошла инициализация сертификата учета, id тот что указан',
            $this->nerferSpecail->certificate->id
        )->equals(self::CERTIFICATE_SPECIAL_ID);
    }

    public function testSpecialCanNotNerfCertificate()
    {
        expect(
            'валидация ПФ сертификата, нерфить можно(по типу сертификата)',
            $this->nerfer->validate(['certificate'])
        )->true();
        expect(
            'валидация сертификата учета, нерфить нельзя(по типу сертификата)',
            $this->nerferSpecail->validate(['certificate'])
        )->false();
    }
    ...
    public function testBallanceValidate()
    {
        ...
        $this->nerfer->certificate->nominal = 555;
        expect(
            'валидация ПФ сертификата, баланс должен быть равен номиналу, а если не равен',
            $this->nerfer->validate(['certificate'])
        )->false();
