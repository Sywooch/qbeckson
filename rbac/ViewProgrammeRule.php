...
class ViewProgrammeRule extends Rule
{
    ...
    public function execute($user, $item, $params)
    {
        ...
        if (Yii::$app->user->can(UserIdentity::ROLE_CERTIFICATE)
            && $programme->municipality->operator_id == $userIdentity->certificate->payer->operator_id
        ) {
            return true;
        }
        if (Yii::$app->user->can(UserIdentity::ROLE_ORGANIZATION)
            && $programme->organization_id == $userIdentity->organization->id
        ) {
            return true;
        }
        if (Yii::$app->user->can(UserIdentity::ROLE_PAYER)) {
            $organizations = $userIdentity->payer->findCooperateOrganizations();
        ...
        if (Yii::$app->user->can(UserIdentity::ROLE_OPERATOR)
            && $programme->municipality->operator_id == $userIdentity->operator->id
        ) {
            return true;
        }
