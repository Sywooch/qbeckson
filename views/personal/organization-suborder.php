    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <?php if (!empty($suborder) && $suborder->status == OrganizationPayerAssignment::STATUS_PENDING): ?>
                <h3>Подтвердите, что вы являетесь подведомственной организацией городского округа (муниципального района): <b><?= $suborder->organization->municipality->name ?> (<?= $suborder->payer->name ?>).</b></h3>
            ...
            <?php elseif (!empty($suborder) && $suborder->status == OrganizationPayerAssignment::STATUS_ACTIVE): ?>
                <h3>Вы указаны в качестве подведомственной организации городского округа (муниципального района): <b><?= $suborder->organization->municipality->name ?> (<?= $suborder->payer->name ?>).</b></h3>
            <?php else: ?>
